...
serve(async (req) => {
  ...
  const customSchemeLink = `midpointmeet://session?sessionId=${encodeURIComponent(sessionId)}&token=${encodeURIComponent(token)}&isSender=false`;
  const intentLink = `intent://session?sessionId=${encodeURIComponent(sessionId)}&token=${encodeURIComponent(token)}&isSender=false#Intent;scheme=midpointmeet;package=com.midpointmeet.app;end`;
